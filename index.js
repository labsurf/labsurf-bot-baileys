...
async function guardarLead(message, producto, estado, origen = '') {
    ...
    if (message.from && message.from.match(/^\d+@s\.whatsapp\.net$/)) {
        ...
        if (/^\+\d+$/.test(posibleNumero)) {
            numeroReal = posibleNumero.replace(/^\+/, '') + '@s.whatsapp.net';
        } else {
            numeroReal = message.from;
        }
    } else {
...
async function enviarCampana(sock) {
    console.log('\n📢 Iniciando campaña...');
    const t0 = Date.now();
    try {
        ...
        for (let i = 0; i < pendientes.length; i++) {
            ...
            try {
                await sock.sendMessage(lead.numero, {
...
async function procesarMensaje(sock, message) {
    ...
    const enviarRespuesta = async (respuesta) => {
        await sock.sendMessage(user, { text: respuesta });
    ...
    if (salesManager.isInSalesFlow(user)) {
        ...
        const responderConRAG = async (pregunta) => {
            ...
            await sock.sendMessage(user, { text: '💭 Consultando...' });
            let contexto = null;
            if (isReady()) {
                try {
...
`;
        ...
        if (decisionIA === 'EXPLICAR') {
            ...
            if (salesState.contadorExplicar >= 2) {
                ...
                await sock.sendMessage(user, { text: '💭 Preparando información...' });
        ...
        switch (decisionIA) {
            ...
            case 'EXPLICAR':
                ...
                await sock.sendMessage(user, { text: '💭 Pensando...' });
            ...
            case 'COMPRA':
                ...
                await sock.sendMessage(user, { text: alternativeMsgCompra });
            ...
            case 'OBJECION':
                ...
                await sock.sendMessage(user, { text: '¿Qué opinas? Responde *SI* para continuar.' });
...
Responde en máximo 3 líneas. Sé natural y cálido.`;
    ...
    if (state.step === STATES.CIERRE) {
        ...
        if (texto.includes('info') || texto.includes('beneficio') || texto.includes('sirve') ||
            ...
            return procesarMensaje(sock, message);
        }
...
async function startBaileys() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            console.log('\n✅ Bot conectado a WhatsApp correctamente (Baileys)');
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (!msg.key.fromMe && msg.message?.conversation) {
            ...
            const messageSimulado = {
                from: user,
                body: texto,
                sender: { pushname: msg.pushName || 'Usuario' },
                isGroupMsg: false,
                fromMe: false
            };

            processingUsers.add(userBase);
            try {
                await procesarMensaje(sock, messageSimulado);
            } catch (err) {
                console.error('❌ Error general:', err.message);
                await sock.sendMessage(user, { text: '❌ Ocurrió un error. Intenta de nuevo.' });
            } finally {
                processingUsers.delete(userBase);
            }
        }
    });
...
setInterval(() => {
    ...
    for (const userId in userState) {
        if (now - userState[userId].lastActivity > 3600000) {
